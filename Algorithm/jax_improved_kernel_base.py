...
class PLSBase(abc.ABC):
    ...
    def compute_regression_coefficients(self, b_last: jnp.ndarray, r: jnp.ndarray, q: jnp.ndarray) -> jnp.ndarray:
        b = b_last + r @ q.T
        return b

    @abc.abstractmethod
    @partial(jax.jit, static_argnums=(0, 1, 2, 3))
    def _get_initial_matrices(self, A, K, M) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    def _fit_helper(self, X: jnp.ndarray, Y: jnp.ndarray, A: int) -> Union[Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray], Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]]:
        pass
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _step_3(self, i: int, w: jnp.ndarray, P: jnp.ndarray, R: jnp.ndarray) -> jnp.ndarray:
    ...
    @partial(jax.jit, static_argnums=0)
    def _step_3_body(self, j: int, carry: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    @partial(jax.jit, static_argnums=0)
    def _step_5(self, XTY: jnp.ndarray, p: jnp.ndarray, q: jnp.ndarray, tTt: jnp.ndarray) -> jnp.ndarray:
    ...
    @partial(jax.jit, static_argnums=(0, 3))
    def stateless_predict(self, X: jnp.ndarray, B: jnp.ndarray, A: None | int = None) -> jnp.ndarray:
    ...
    def stateless_fit_predict_eval(self, X_train: jnp.ndarray, Y_train: jnp.ndarray, A: int, X_test: jnp.ndarray, Y_test: jnp.ndarray, metric_function: Callable[[jnp.ndarray, jnp.ndarray], Any]) -> Tuple[jnp.int64, jnp.float64]:
        """
        Parameters:
        X: Predictor variables matrix (N x K)
    ...
    def _inner_loocv(self, i: int, X_train_val: jnp.ndarray, Y_train_val: jnp.ndarray, A: int, metric_function: Callable[[jnp.ndarray, jnp.ndarray], Any]) -> Any:
        print('Tracing inner LOOCV...')
        all_indices = jnp.arange(X_train_val.shape[0])
        train_indices = jnp.nonzero(
            all_indices != i, size=X_train_val.shape[0] - 1)[0]
    ...
    def loocv(self, X_train_val: jnp.ndarray, Y_train_val: jnp.ndarray, A: int, metric_function: Callable[[jnp.ndarray, jnp.ndarray], Any], metric_names: list[str]) -> dict[str]:
        ...
        for i in tqdm(range(1, X_train_val.shape[0]), initial=1, total=X_train_val.shape[0]):
            metric_values = self._inner_loocv(
                i, X_train_val, Y_train_val, A, metric_function)
            metric_value_lists = self.update_metric_value_lists(
                metric_value_lists, metric_values)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _sample_with_replacement(self, prng_key: Union[jnp.ndarray, jax.random.PRNGKeyArray], X_train: jnp.ndarray, Y_train: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
    ...
    def bootstrap(self, seed: int, num_iters: int, A: int, X_train: jnp.ndarray, Y_train: jnp.ndarray,  X_val: jnp.ndarray, Y_val: jnp.ndarray, metric_function: Callable[[jnp.ndarray, jnp.ndarray], Any], metric_names: list[str]) -> dict[str]:
        metric_value_lists = [[] for _ in metric_names]
