            ...
            if Log_plt_norm!=None:
                if Log_plt_norm ==1:
                    ...
                    plt.semilogy(xx,MV_t[ii,:]*100/YT[n_idx,MVs_idx[ii]], Marker_1, markersize=5, 
                               ...
                               label=f'Pred. Resuts for MV#{ii+1} from sample #{n_idx}')
                    if LV_param!=None:
                        LV_cnt = LV_param[0]
...
def PLS2Based_Imputation(XI, YI1, App, Just_do_min, Opt_LV, Max_LV, cv_mode, 
                         ...
                         Thresh_itr = None, CNT=None, Thresh = None):
    # App: 
    #1) 'A1xy': 'A1xy' --> Update per sample from the lowest no. MVs to the highest number
    #2) 'A2xy': 'A2xy' --> update all var.s in each strat. once at a time
    #3) 'A3xy': 'A3xy' --> update the var.s in each substrat. once at a time
    ###########################################################################
        ...
        if App == 'A2xy' or App == 'A3xy':
            for ii in range(len(KEYS1)):
                ...
                if ii == len(KEYS1)-1:
                    ...
                    predP1[np.where(missingmap_yi)]=pred[np.where(missingmap_yi)]
