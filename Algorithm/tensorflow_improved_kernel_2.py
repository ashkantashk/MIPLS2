...
class PLS():
    ...
    def helper(self, XTX, XTY):
        ...
        while tf.less(self.i, self.A):
            ...
            # step 4
            rT = tf.transpose(self.r)
            rTXTX = tf.matmul(rT, XTX)
            tTt = tf.matmul(rTXTX, self.r)
            p = tf.transpose(rTXTX) / tTt
    ...
    @tf.function#(jit_compile=True)
    def _get_initial_tensors(self, A, K, M, dtype):
    ...
    def fit(self, X, Y, A):
        ...
        if A != self.A or N != self.N or K != self.K or M != self.M or dtype != self.dtype:
            ...
            self.B, self.W, self.P, self.Q, self.R, self.i, self.j, self.r = map(tf.Variable, self._get_initial_tensors(A, K, M, dtype))
