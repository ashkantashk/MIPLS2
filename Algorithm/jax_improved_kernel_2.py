...
class PLS(PLSBase):
    ...
    def _get_initial_matrices(self, X: jnp.ndarray, Y: jnp.ndarray, A: int) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        print('Tracing initial matrices...')
        return super()._get_initial_matrices(X, Y, A)

    @partial(jax.jit, static_argnums=(0,))
    def _step_1(self, X: jnp.ndarray, Y: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
    ...
    @partial(jax.jit, static_argnums=(0,))
    def _step_4(self, XTX: jnp.ndarray, XTY: jnp.ndarray, r: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    def _main_loop_body(self, i: int, XTX: jnp.ndarray, XTY: jnp.ndarray, M: int, K: int, P: jnp.ndarray, R: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    ...
    def fit(self, X: jnp.ndarray, Y: jnp.ndarray, A: int) -> None:
        ...
        """
        self.B, self.W, self.P, self.Q, self.R = self._fit_helper(X, Y, A)

    @partial(jax.jit, static_argnums=(0, 3))
    def _fit_helper(self, X: jnp.ndarray, Y: jnp.ndarray, A: int) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        """
        Parameters:
        X: Predictor variables matrix (N x K)
        Y: Response variables matrix (N x M)
